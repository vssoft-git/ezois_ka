...
Функция СведенияОбОрганизации(Знач Организация, Знач ИменаПолей, Знач ДатаПолучения = Неопределено) Экспорт
	...
	КонецЕсли;
	Если Результат.Свойство("ВключенаПередачаНДФЛЧерезБанкАУСН")
		И ТипЗнч(Результат.ВключенаПередачаНДФЛЧерезБанкАУСН) <> Тип("Булево") Тогда
		Результат.ВключенаПередачаНДФЛЧерезБанкАУСН = Ложь;
	КонецЕсли;
