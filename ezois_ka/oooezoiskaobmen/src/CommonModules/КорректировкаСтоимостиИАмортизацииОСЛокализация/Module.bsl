...
// Возвращаемое значение:
//  Булево - 
Функция ЗаполнитьДокументНаОснованииИзменениеПараметровОС(Объект, Основание) Экспорт
...
Функция ЗаполнитьДляПримененияИНВ(Объект, Основание)
	...
	|		СтоимостьОСОбороты.АмортизационнаяПремияРасход КАК СуммаАмортизационнойПремии
	|	ИЗ
	...
	СуммаАмортизационнойПремии = ?(АмортизационнаяПремия.Количество() <> 0, АмортизационнаяПремия[0].СуммаАмортизационнойПремии, 0);
	...
	Если ВыборкаРеквизиты.ИнвестиционныйНалоговыйВычет = Перечисления.ВариантыПримененияИНВ.Федеральный Тогда
		...
		Для Сч = 1 По Объект.ПараметрыЗаПериодБУ.Количество() Цикл
			...
			Иначе	
				...
				Если СтрокаНУ <> Неопределено Тогда
					СтоимостьНУ = СтрокаНУ.СтоимостьНУ + СуммаАмортизационнойПремии;
					КорректировкаСтоимостиИНВ = Окр(СтоимостьНУ * ДоляРасходовИНВ / 100, 2);
					ДанныеСтроки.СтоимостьПР = ДанныеСтроки.СтоимостьПР + КорректировкаСтоимостиИНВ;
				КонецЕсли;
		...
		Для Сч = 1 По Объект.ПараметрыЗаПериодНУ.Количество() Цикл
			...
			Иначе
				СтоимостьНУ = ДанныеСтроки.СтоимостьНУ + СуммаАмортизационнойПремии;
			КонецЕсли;
		...
		Для Каждого ДанныеСтроки Из Объект.ОС Цикл
			...
			Если ДанныеСтроки.АмортизацияНУ <> ИсходныеЗначения.АмортизацияНУ
				ИЛИ ДанныеСтроки.АмортизацияПР <> ИсходныеЗначения.АмортизацияПР Тогда
	...
	Если ВыборкаРеквизиты.ИнвестиционныйНалоговыйВычет = Перечисления.ВариантыПримененияИНВ.Региональный Тогда
		...
		Для Сч = 1 По Объект.ПараметрыЗаПериодБУ.Количество() Цикл
			...
			Иначе	
				...
				СтрокаНУ = Объект.ПараметрыЗаПериодНУ.Найти(НачалоДня(КонецМесяца(ДанныеСтроки.Дата)), "Дата");
				Если СтрокаНУ <> Неопределено Тогда
					КорректировкаСтоимостиИНВ = СтрокаНУ.СтоимостьНУ + СуммаАмортизационнойПремии;
					ДанныеСтроки.СтоимостьПР = ДанныеСтроки.СтоимостьПР + КорректировкаСтоимостиИНВ;
				КонецЕсли;
		...
		Для Каждого ДанныеСтроки Из Объект.ОС Цикл
			...
			КорректировкаСтоимостиИНВ = ДанныеСтроки.СтоимостьНУ + СуммаАмортизационнойПремии;
			...
			Если СуммаАмортизационнойПремии <> 0 Тогда
				ДанныеСтроки.АмортизационнаяПремия = СуммаАмортизационнойПремии;
