...
Процедура ЗаполнитьНастройкиПоУмолчаниюОДДС(Процессор)
	...
	ДобавитьПоказатель(Процессор, НСтр("ru = 'Поступления от продаж по платежным картам'", КодЯзыка));
	...
	ДобавитьПоказательРаспределенногоДенежногоПотока(
		...
		НСтр("ru = 'НДФЛ с оплаты труда, зачисленный на единый налоговый счет'", КодЯзыка));
	ДобавитьПоказательРаспределенногоДенежногоПотока(
		Процессор,
		"ЕНС",
		Перечисления.ВидыНалогов.НалоговыеПлатежиСтраховыеВзносыОплатыТруда(),
		НСтр("ru = 'Страховые взносы с оплаты труда, зачисленные на единый налоговый счет'", КодЯзыка));
	...
	ДобавитьПоказательРаспределенногоДенежногоПотока(
		...
		НСтр("ru = 'Налог на прибыль, зачисленный на единый налоговый счет'", КодЯзыка));
...
Процедура ОписатьПорядокЗаполненияПоказателяДенежныеПотоки(Описание, Показатель, ДанныеАлгоритма, КодЯзыка)
	...
	Иначе
		...
		Если Не ЗначениеЗаполнено(Сворачиваемые) Тогда
			Предложения.Добавить(НСтр("ru = 'Исключаются обороты между счетами денежных средств и денежных эквивалентов.'", КодЯзыка));
		Иначе
			...
			КонецЕсли;
			Союз = НСтр("ru = ', а также '");// используем нестандартный союз, так как описания сворачиваемых потоков уже содержат союз "и"
