...
Процедура ПодготовитьНастройки(Заголовки)
	...
	Если НЕ НастройкиКС.Критичность = 3 Тогда
		...
		НастройкиКС.Основание 	= "Проверка носит справочный характер";
	КонецЕсли;
...
Процедура ПроверкаКС_2_25_доп()
	...
	Заголовки.Вставить("Описание"			, 
		...
		НСтр("ru='если стр.100 Листа 02 > 0 и признак налогоплательщика ≠ 05, 51'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = %2'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная сумма налоговой базы'"));
	...
	Для каждого Лист02 Из ДанныеКС.Лист02 Цикл
		...
		Для каждого Лист02_4 Из Лист02.Лист02_4 Цикл
			...
			НастройкиКС.ЭтоОшибка = Лист02_4.РазделЗаполнен_Лист02_4 И (ДанныеКС.МесОтчета = 3 ИЛИ ДанныеКС.МесОтчета = 12) 
				И ДанныеКС.ЭтоГоловная И НЕ КодНайден(Показатели.Л02С001.Значение, "05,51") 
...
Процедура ПроверкаКС_2_26_доп()
	...
	Заголовки.Вставить("Описание"			, 
		...
		НСтр("ru='признак налогоплательщика ≠ 05, 51'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная сумма налоговой базы'"));
...
Процедура ПроверкаКС_2_29_доп()
	...
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Сумма убытка в стр.150 Приложения 4 к Листу 02 <= стр.010 Приложения 4 к Листу 02, '") +
	...
	Для каждого Лист02 Из ДанныеКС.Лист02 Цикл
		Для каждого Лист02_4 Из Лист02.Лист02_4 Цикл
			...
			НастройкиКС.ЭтоОшибка = (ДанныеКС.МесОтчета = 3 ИЛИ ДанныеКС.МесОтчета = 12) 
				...
				И Показатели.П24С001.Значение > Показатели.П24С003.Значение;
			НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
				Показатели.П24С001.Значение, Показатели.П24С003.Значение, Показатели.П24С004.Значение);
			ДополнитьТаблицуРезультата();
...
Процедура ПроверкаКС_2_31_доп()
	...
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Сумма убытка в стр.160 Приложения 4 к Листу 02 = стр.010 - стр.150 '") +
		НСтр("ru='Приложения 4 к Листу 02, если стр.100 Листа 02 >= 0'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = %4 (%2 - %3)'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная сумма убытка'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните сумму убытка в стр.160 Приложения 4 к Листу 02");
	Заголовки.Вставить("Основание"			, НСтр("ru='Проверка носит справочный характер'"));
	...
	Для каждого Лист02 Из ДанныеКС.Лист02 Цикл
		...
		Для каждого Лист02_4 Из Лист02.Лист02_4 Цикл
			...
			Если НастройкиКС.БольшеНуля Тогда
				...
				Разница1 = Показатели.П24С003.Значение - Показатели.П24С004.Значение;
				НастройкиКС.ЭтоОшибка = НастройкиКС.ЭтоОшибка И Показатели.Л02С001.Значение >= 0 И НЕ Показатели.П24С001.Значение = Разница1;
				НастройкиКС.Детализация = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НастройкиКС.ШаблонДетализации, 
					Показатели.П24С001.Значение, Показатели.П24С003.Значение, Показатели.П24С004.Значение, Разница1);
					
			Иначе
					
				Разница2 = Показатели.П24С003.Значение - Показатели.Л02С001.Значение;
				НастройкиКС.ЭтоОшибка = НастройкиКС.ЭтоОшибка И Показатели.Л02С001.Значение < 0 И НЕ Показатели.П24С001.Значение = Разница2;
				НастройкиКС.Детализация = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НастройкиКС.ШаблонДетализации, 
					Показатели.П24С001.Значение, Показатели.П24С003.Значение, Показатели.Л02С001.Значение, Разница2);
...
Процедура ПроверкаКС_2_32_доп()
	...
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Сумма убытка в стр.160 Приложения 4 к Листу 02 = стр.010 - стр.100 '") +
	...
	Заголовки.Вставить("Основание"			, НСтр("ru='Проверка носит справочный характер'"));
	Заголовки.Вставить("Условие"			, НСтр("ru='Ошибка выдается только в отчете за год'"));
	Заголовки.Вставить("ДляДокументации"	, НСтр("ru='Отключено с 2026'"));
	ПодготовитьНастройки(Заголовки);
